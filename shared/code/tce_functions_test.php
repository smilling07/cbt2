...
{
	$genset = unserialize(file_get_contents('../../public/config/tmf_greetings.json'));
	if($genset['enable_greeting']){
		echo '<div class="p-1em">'.K_NEWLINE;
		// echo '<div class="p-05em">';
		echo '<span class="d-block">'.$genset['greetLine1'].'<span class="ft-500 ft-style-i">'.urldecode($_SESSION['session_user_firstname']).'</span></span>';
		echo '<span class="d-block mt-5 py-05em brad5 px-1em" style="background:var(--col-12);border-left:3px solid var(--col-2)">'.$genset['greetLine2'].'</span>';
	}
        ...
        while ($m = F_db_fetch_array($r)) { // for each active test
            ...
            if (F_isValidTestUser($m['test_id'], $_SESSION['session_user_ip'], $m['test_ip_range'])) {
                ...
                if (strtotime($current_time) >= strtotime($m['test_end_time'])) {
                    ...
                    $lockicon = ' <span class="icon-lock" style="background:var(--col-10);color:var(--col-7);padding:0.25em;border-radius:50em"></span>';
                } else {
                    $datestyle = '';
                    $lockicon = '';
                }
                // $str .= '<ul style="border-top:3px solid '.K_NEWLINE;
                $str .= '<ul class="';
				if (!$expired) {
					switch ($test_status) {
                        case 0: {
                ...
                if (strlen($m['test_password']) > 0) {
					...
					$testpw = 'testpw=1';
                    $str .= '<li>';
					$passicon = ' <i class="icon-key" style="background:var(--col-4t);color:var(--col-4);padding:0.25em;border-radius:50em"></i>';
                } else {
					$istestpw = 0;
					$testpw = 'testpw=0';
                    $str .= '<li>';
                ...
                $str .= '<div><strong>'.F_testInfoLink($m['test_id'], $m['test_name']).$lockicon.$passicon.'</strong></div>'.K_NEWLINE;
				$str .= '<div>'.K_NEWLINE;
                ...
                if (!$expired) {
                    switch ($test_status) {
                        case 0: { // 0 = the test generation process is started but not completed
                            ...
                            $str .= '?testid='.$m['test_id'].'&'.$testpw.'\')" title="'.$l['h_execute'].'" class="buttongreen"><span class="icon-chevron-circle-right"></span> '.$l['w_execute'].'</a>';
                            break;
                        }
                        ...
                        case 3: { // 3 = all questions have been answered
                            // continue test
                            $str .= '<a id="'.$testuser_id.'" onclick="reloadCont.style.display=\'block\';backdrop(\'1\',\'1\');window.location.replace(\'tce_test_execute.php?testid='.$m['test_id'].'\')" title="'.$l['h_continue'].'" class="xmlbutton continue"><span class="icon-spinner anim-rotate d-iblock"></span> '.$l['w_continue'].'</a>';
                            break;
                        }
						case 9: {
							/* if($istestpw===1){
				...
				$str .= '</div>'.K_NEWLINE;
				$str .= '</li>'.K_NEWLINE;
                $str .= '<li'.$datestyle.'><span>'.$l['w_from'].'</span><span class="d-flex jc-c"><span class="icon-calendar-o" style="color:var(--col-4)"></span><span style="margin-left:0.25em;font-size:0.97em">'.date('d M Y - H:i:s',strtotime($m['test_begin_time'])).'</span></li>'.K_NEWLINE;
                $str .= '<li'.$datestyle.'><span>'.$l['w_to'].'</span><span class="d-flex jc-c"><span class="icon-calendar" style="color:var(--col-4)"></span><span style="margin-left:0.25em;font-size:0.97em">'.date('d M Y - H:i:s',strtotime($m['test_end_time'])).'</span></li>'.K_NEWLINE;
                // status
                $str .= '<li';
                // if ((( $test_status > 4 and $test_status < 9 ) or $test_status > 9 ) and F_getBoolean($m['test_results_to_users'])) {
                if (($test_status >= 4 and F_getBoolean($m['test_results_to_users'])) and $test_status!=9) {
                    ...
                    if (isset($usrtestdata['user_score']) and isset($usrtestdata['test_score_threshold']) and ($usrtestdata['test_score_threshold'] > 0)) {
                        if ($usrtestdata['user_score'] >= $usrtestdata['test_score_threshold']) {
                            ...
                            $bg = ' style="display:inline-block;font-weight:500;color:var(--col-9)"';
                            $passmsg = '<span class="d-flex brad50 p-05em jc-c"><span class="icon-star" style="color:var(--col-9)"></span>&nbsp;<span style="color:#2b7b60;font-weight:500">'.$l['w_passed'].'</span></span>';
                        } else {
                            // $bg = ' style="display:inline-block;background-color:var(--col-10t);color:var(--col-11)"';
                            $bg = ' style="display:inline-block;font-weight:500;color:var(--col-10)"';
                            $passmsg = '<span class="d-flex brad50 p-05em jc-c"><span class="icon-warning" style="color:var(--col-10)"></span>&nbsp;<span style="color:var(--col-10);font-weight:500">'.$l['w_not_passed'].'</span></span>';
                        }
                    }
                    ...
                    if (isset($usrtestdata['user_score']) and (strlen(''.$usrtestdata['user_score']) > 0)) {
                        if ($usrtestdata['test_max_score'] > 0) {
                            $str .= '<a href="#" '.$bg.' onclick="reloadCont.style.display=\'block\';backdrop(\'1\',\'1\');window.location.replace(\'tce_show_result_user.php?testuser_id='.$testuser_id.'&amp;test_id='.$m['test_id'].'\')" title="'.$l['h_result'].'">'.$usrtestdata['user_score'].' / '.$usrtestdata['test_max_score'].' ('.round(100 * $usrtestdata['user_score'] / $usrtestdata['test_max_score']).'%)'.$passmsg.'</a>';
                        } else {
                            $str .= '<a href="#" '.$bg.' onclick="reloadCont.style.display=\'block\';backdrop(\'1\',\'1\');window.location.replace(\'tce_show_result_user.php?testuser_id='.$testuser_id.'&amp;test_id='.$m['test_id'].'\')" title="'.$l['h_result'].'">'.$usrtestdata['user_score'].$passmsg.'</a>';
                        }
                    } else {
                        $str .= '</span>';
                    }
...
function F_printTestInfo($test_id, $showip = false)
{
    ...
    if ($r = F_db_query($sql, $db)) {
        if ($m = F_db_fetch_array($r)) {
            ...
            $str .= F_twoColRow($l['w_time_begin'], $l['h_time_begin'], $m['test_begin_time'], '<span class="icon-calendar-o"></span>');
            ...
            $str .= F_twoColRow($l['w_score_wrong'], $l['h_score_wrong'], $m['test_score_wrong'], '<span class="icon-dangerous"></span>');
            $str .= F_twoColRow($l['w_score_unanswered'], $l['h_score_unanswered'], $m['test_score_unanswered'], '<span class="icon-file-empty"></span>');
            $str .= F_twoColRow($l['w_max_score'], $l['w_max_score'], $m['test_max_score'], '<span class="icon-star"></span>');
...
/**
 * Returns the test end time.
 * @param $test_id (int) test ID.
 * @return string test_end_time or empty string in case of error.
 */
function F_getTestEndTime($test_id)
{
    $test_id = intval($test_id);
    $td = F_getTestData($test_id);
    return $td['test_end_time'];
}
...
{	
    ...
    if ($answer_changed or ($oldtext != $answer_text)) {
        ...
        if ($unanswered) {
            ...
            $change_time1 = strtotime(date(K_TIMESTAMP_FORMAT))+($reaction_time/1000);
            $change_time = date(K_TIMESTAMP_FORMAT,$change_time1);
            // $change_time = date(K_TIMESTAMP_FORMAT,$reaction_time);
        }
		
		//jika tidak ingin dinilai secara realtime baris ini dikomen atau buatkan kondisi khusus 
...
function F_updateQuestionLogRegradeOld($test_id, $testlog_id, $answpos = array(), $answer_text = '', $reaction_time = 0)
{
	if($test_id===0){
...
function F_questionForm($test_id, $testlog_id, $formname)
{
	...
	// if(!isset($_POST['question-block'])){
		// echo '<div id="test_end_time" class="hidden">'.$testdata['test_end_time'].'</div>';
	// }
    ...
    if ($r = F_db_query($sql, $db)) {
        if ($m = F_db_fetch_array($r)) {
			...
			if(!isset($_POST['question-block'])){
				if(!file_exists(K_PATH_MAIN.'answdata/'.$m['testlog_testuser_id'].'_answdata.txt') and QBLOCK_JSON){
					$fpad = fopen(K_PATH_MAIN.'answdata/'.$m['testlog_testuser_id'].'_answdata.txt', 'w');
					fwrite($fpad, '[]');
					fclose($fpad);
				}
				if(!file_exists(K_PATH_MAIN.'answdata/'.$m['testlog_testuser_id'].'_unsure.txt')){
					$fpad = fopen(K_PATH_MAIN.'answdata/'.$m['testlog_testuser_id'].'_unsure.txt', 'w');
					fwrite($fpad, '[]');
					fclose($fpad);
				}
				...
				if (F_getBoolean($test_data['test_logout_on_timeout'])) {
					...
					$str .= '<input type="hidden" name="timeout_logout" id="timeout_logout" value="0" />'.K_NEWLINE;
				}
				$str .= '<a name="questionsection" id="questionsection"></a>'.K_NEWLINE;
					// echo $test_id;
					// echo $m['testlog_testuser_id'];
					if(GENSS_QBLOCK_JSON){
						require_once('../../shared/code/tmf_qjson_data.php');
						// echo '<script>'.K_NEWLINE;
						F_printUserTestStat($m['testlog_testuser_id'],$test_id);
						// echo '</script>'.K_NEWLINE;
					}
					if(isset($_GET['xxx'])){
						require_once('../../shared/code/tmf_qjson_data.php');
						// echo '<script>'.K_NEWLINE;
						F_printUserTestStat($_GET['testuserid'],$_GET['testid']);
						// echo '</script>'.K_NEWLINE;
					}
					// sleep(5);
			}
			...
			if(file_exists(K_PATH_QBLOCK.$testlog_id.'.json') && QBLOCK_JSON){
				if(isset($_POST['question-block'])){
					// echo 'asdasd';
				...
				if(QBLOCK_JSON){
					...
					if ($m['question_type'] == 3) {
						...
						$qblock_arr['content'] .= '<a class="xmlbutton buttonblue btn-outlined" name="editorLoaderBtn" id="editorLoaderBtn" onclick="loadWYSIBB()"><span class="icon-wysiwyg"></span> '.$l['a_load_bbeditor'].'</a>'.K_NEWLINE;
						...
						if(K_PUBLIC_FILE_UPLOAD){
							...
							$qblock_arr['content'] .= '<div id="btn_uploadFileCont" style="display:none"><a onclick="window.open(\'tce_select_userphoto.php?frm=testform&amp;fld=answertext,\',\'mediaselect\',\'height=600,width=680,resizable=yes,menubar=no,scrollbars=yes,toolbar=no,status=no,modal=yes\');" id="btn_uploadFile" class="xmlbutton buttonblue"><span class="icon-upload"></span> '.$l['w_upload_file'].' / '.$l['b_media_insert'].'</a>&nbsp;<a id="imgProblem" href="#imgProblem">'.$l['a_img_ifnodisplay'].'</a></div>'.K_NEWLINE;
						}
					} else {
...
function F_questionsMenu($testdata, $testuser_id, $testlog_id = 0, $disable = false)
{
	...
	if(!isset($_POST['question-block'])){
		...
		$navlink .= '<button ';
		if(!K_SHOW_SAVE_BUTTON){
			$navlink .= 'style="display:none" ';
		}
		$navlink .= 'id="relbtn" onclick="saveAnswerAjax(0,this)" type="button" title="'.$l['w_save'].'">'.$l['w_save'].'</button>';
		...
		$navlink .= '&nbsp;<button id="termbtn" onclick="saveAnswerAjax(0,this)" type="button" title="'.$l['w_terminate'].'">'.$l['w_terminate'].'</button>';
...
if(!isset($_POST['question-block'])){	
	...
	echo '<div id="nosoalCont" class="d-flex ai-unset jc-se fwrap"><div id="ns1" class="d-flex ai-unset"><span id="fontplus" onclick="zoomintext()">&plus;</span><span id="fontminus" onclick="zoomouttext()">&minus;</span><span id="nosoal">'.$qsel.'</span></div><div id="ns-center" class="d-flex ai-unset"><span id="darkModeBtn" onclick="darkMode()"><span id="darkModeLbl">'.$l['w_to_dark'].'</span></span><span id="lightModeBtn" onclick="lightMode()"><span id="lightModeLbl">'.$l['w_to_light'].'</span></span><span id="fullScBtn" onclick="openFullScr()"><span class="icon-fullscreen-alt"></span><span id="fullScBtnLbl" class="hidden show768">&nbsp;'.ucfirst($l['w_fullscreen']).'</span></span><span id="resScBtn" onclick="closeFullScr()" style="display:none"><span class="icon-close_fullscreen"></span><span id="resScBtnLbl" class="hidden show768">&nbsp;'.ucfirst($l['w_close'].' '.$l['w_fullscreen']).'</span></span></div><div id="ns2" class="d-flex ai-unset"><span ';
	if(!QBLOCK_JSON or !CACHE_FEATURE){echo 'class="hidden"';}
	echo 'id="cacheBtn" onclick="cacheToggle()"><span class="icon-file-empty"></span> <span id="cacheLbl" class="hidden show768">'.ucfirst($l['w_cache']).'</span></span><span id="reloadBtn" onclick="reloadCont.style.display=\'block\';backdrop(\'100\',\'1\');location.reload()"><span class="icon-spinner11"></span> <span id="reloadLbl" class="hidden show768">'.ucfirst($l['w_reload']).'</span></span>';
	if (F_getBoolean($testdata['test_comment_enabled']) and (!$disable)){
...
function F_testLoginForm($faction, $fid, $fmethod, $fenctype, $test_id)
{
    ...
    $str .= getFormRowTextInputIcon('xtest_password', $l['w_test_password'], $l['h_test_password'].'" class="lh2', '', '', '', 255, false, false, true, '', 'pos-rel', '<span class="icon testpwdicon icon-eye" id="showPass"></span><span class="icon testpwdicon icon-eye-blocked" id="hidePass"></span>');
    // buttons
    $str .= '<div class="row">'.K_NEWLINE;
    $str .= '<button onclick="reloadCont.style.display=\'block\';backdrop(\'1\',\'1\');setTimeout(function(){document.getElementById(\'sublogin\').click()},K_ENABLE_DELAY)" type="button" name="login" id="login" title="'.$l['h_login_button'].'">'.$l['w_login'].'</button>'.K_NEWLINE;
    $str .= '<input class="hidden" type="submit" name="sublogin" id="sublogin" value="'.$l['w_login'].'" title="'.$l['h_login_button'].'" />'.K_NEWLINE;
