...
{
    echo '<input type="submit" class="mb-2 mr-2 btn btn-primary text-capitalize" name="'.$name.'" id="'.$name.'" value="'.$value.'" title="'.$title.'" />';
}

/* function F_submit_button_altx($name, $value, $title = "", $altx = "mb-2 mr-2 btn btn-dark")
{
    echo '<input type="submit" class="'.$altx.'" name="'.$name.'" id="'.$name.'" value="'.$value.'" title="'.$title.'" />';
} */

function F_submit_button_alt($name, $value, $title = "", $class = "mb-2 mr-2 btn btn-primary")
{
    echo '<input type="submit" class="'.$class.' text-capitalize" data-toggle="tooltip" name="'.$name.'" id="'.$name.'" value="'.$value.'" title="'.$title.'" />';
}
...
function TMF_tooltip_button($label,$title,$class){
	$res = '<button type="button" data-toggle="tooltip" class="'.$class.'" data-original-title="'.$title.'" title="'.$title.'">'.$label.'</button>';
	return $res;
}

function TMF_modal_button($id,$btnText,$class){
	$res = '<button type="button" class="'.$class.'" data-toggle="modal" data-target="#'.$id.'">'.$btnText.'</button>';
	return $res;
}

function TMF_modal_info($id, $title, $text, $onclose = ''){
$res = '<div class="modal fade" id="'.$id.'" tabindex="2" role="dialog" aria-labelledby="'.$id.'Label" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="'.$id.'Label"><i class="fa fa-info-circle"></i> '.$title.'</h5>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal" onclick="'.$onclose.'">Tutup</button>
            </div>
        </div>
    </div>
</div>';
 ...
 * @return string
 */
function getFormRowTextInput($field_name, $name, $description = '', $tip = '', $value = '', $format = '', $maxlen = 255, $date = false, $datetime = false, $password = false, $prefix = '', $group = false)
{
	...
	if($group){
		...
		$divclass = 'position-relative form-group mb-2';
		$labelclass = '';
	}
    $str .= '<div class="'.$divclass.'">'.K_NEWLINE;
    // $str .= '<span class="label">'.K_NEWLINE;
	if($group){
		$str .= '<div class="input-group-prepend">'.K_NEWLINE;
	}
    $str .= '<label class="'.$labelclass.'" id="'.$field_name.'Lbl" for="'.$field_name.'" title="'.$description.'">'.ucfirst($name).'</label>'.K_NEWLINE;
	if($group){
		$str .= '</div>'.K_NEWLINE;
	}
    if (!empty($prefix)) {
    ...
    $str .= ' name="'.$field_name.'" id="'.$field_name.'" value="'.htmlspecialchars($value, ENT_COMPAT, $l['a_meta_charset']).'" size="20" maxlength="'.$maxlen.'" title="'.$description.'" data-toggle="tooltip" class="form-control" />';
    $str .= $button;
	...
	if (strlen($tip) > 0) {
        ...
        $str .= ' <span class="'.$mtval.' d-flex justify-content-end small font-italic text-info">'.$tip.'</span>';
    }
	if (strlen($format) > 0) {
...
function getFormRowTextInputIcon($field_name, $name, $description = '', $tip = '', $value = '', $format = '', $maxlen = 255, $date = false, $datetime = false, $password = false, $prefix = '', $class = '', $tag = '')
{
    require_once(dirname(__FILE__).'/../config/tce_config.php');
    global $l;
    if (strlen($description) == 0) {
    ...
    $str .= '<div class="row">'.K_NEWLINE;
    $str .= '<span class="label">'.K_NEWLINE;
    $str .= '<label id="'.$field_name.'Lbl" for="'.$field_name.'" title="'.$description.'">'.$name.'</label>'.K_NEWLINE;
    if (!empty($prefix)) {
        $str .= $prefix;
    }
    $str .= '</span>'.K_NEWLINE;
    $str .= '<span class="formw '.$class.'">'.K_NEWLINE;
    $str .= '<input type="';
    if ($password) {
        $str .= 'password';
    } else {
...
function getFormRowTextBox($field_name, $name, $description = '', $value = '', $disabled = false, $prefix = '')
{
    ...
    $str = ''; // string to return
    $str .= '<label for="'.$field_name.'" data-toggle="tooltip" title="'.$description.'">'.$name.'</label>'.K_NEWLINE;
    if (!empty($prefix)) {
        $str .= $prefix;
    }
    $str .= '<textarea class="form-control" data-toggle="tooltip" cols="50" rows="5" name="'.$field_name.'" id="'.$field_name.'" title="'.$description.'"';
    if ($disabled) {
 ...
 * @return string
 */
function getFormRowSelectBox($field_name, $name, $description = '', $tip = '', $value = '', $items = array(), $prefix = '', $group = false)
{
    require_once(dirname(__FILE__).'/../config/tce_config.php');
    global $l;
    ...
    // $str .= '<div class="position-relative form-group">'.K_NEWLINE;
    // $str .= '<span class="label">'.K_NEWLINE;
    $str .= '<label class="'.$labelclass.'" for="'.$field_name.'" title="'.$description.'">'.ucfirst($name).'</label>'.K_NEWLINE;
	if($group){
		$str .= '</div>'.K_NEWLINE;
	}
    if (!empty($prefix)) {
    ...
    // $str .= '</span>'.K_NEWLINE;
    // $str .= '<span class="formw">'.K_NEWLINE;
    $str .= '<select class="custom-select" name="'.$field_name.'" id="'.$field_name.'" title="'.$description.'">'.K_NEWLINE;
...
function getFormRowCheckBox($field_name, $name, $description = '', $tip = '', $value = '', $selected = false, $disabled = false, $prefix = '')
{
    ...
    $str = ''; // string to return
    $str .= '<div class="d-block">'.K_NEWLINE;
    $str .= '<div class="custom-checkbox custom-control d-inline-flex flex-column-reverse">'.K_NEWLINE;
    $hidden = '';
    if ($disabled) {
        // add hidden field to be submitted
        $hidden = '<input class="custom-control-input" type="hidden" name="'.$field_name.'" id="'.$field_name.'" value="'.htmlspecialchars($value, ENT_COMPAT, $l['a_meta_charset']).'" />'.K_NEWLINE;
        $field_name = 'DISABLED_'.$field_name;
    }
    if (!empty($prefix)) {
        $str .= $prefix;
    }
    $str .= '<input class="custom-control-input" type="checkbox"';
    if ($disabled) {
        $str .= ' readonly="readonly" class="disabled"';
    }
	...
	$str .= '<label data-toggle="tooltip" class="custom-control-label" for="'.$field_name.'" title="'.$description.'">'.$name.'</label>'.K_NEWLINE;
    $str .= '</div>'.K_NEWLINE;
    $str .= '</div>'.K_NEWLINE;
    return $str;
}
 ...
 * @return string
 */
function getFormRowFixedValue($field_name, $name, $description = '', $tip = '', $value = '', $currency = false, $prefix = '', $group = false)
{
    require_once(dirname(__FILE__).'/../config/tce_config.php');
    global $l;
    if (strlen($description) == 0) {
    ...
    $str .= '<label class="'.$labelclass.'" for="DISABLED_'.$field_name.'" title="'.$description.'">'.ucfirst($name).'</label>'.K_NEWLINE;
	if($group){
		$str .= '</div>'.K_NEWLINE;
	}
    if (!empty($prefix)) {
    ...
    // $str .= '</span>'.K_NEWLINE;
    // $str .= '<span class="formw">'.K_NEWLINE;
    $str .= '<input class="form-control" type="text" readonly="readonly" name="DISABLED_'.$field_name.'" id="DISABLED_'.$field_name.'"';
    if ($currency) {
...
function getFormDescriptionLine($name, $description = '', $value = '')
{
    ...
    $str = '<div class="rowlll">'.K_NEWLINE;
    $str .= '<span class="label">'.K_NEWLINE;
...
function getFormDescriptionLine_alt($name, $description = '', $value = '', $custom = '')
{
    if (strlen($description) == 0) {
        $description = $name;
    }
    $str = '<div class="'.$custom.'">'.K_NEWLINE;
    $str .= '<div class="rowlll">'.K_NEWLINE;
    $str .= '<span class="label">'.K_NEWLINE;
    $str .= '<span title="'.$description.'">'.$name.'</span>'.K_NEWLINE;
    $str .= '</span>'.K_NEWLINE;
...
function getFormDescriptionLine_alt2($name, $description = '', $value = '', $custom = '', $classBadge = '')
{
    if (strlen($description) == 0) {
        $description = $name;
    }
    ...
    $str .= '<span title="'.$description.'">'.$name.'</span>'.K_NEWLINE;
    $str .= '</span>'.K_NEWLINE;
    $str .= '<span class="'.$classBadge.'">'.K_NEWLINE;
    $str .= $value.'&nbsp;'.K_NEWLINE;
    $str .= '</span>'.K_NEWLINE;
    $str .= '</div>'.K_NEWLINE;
    $str .= '</div>'.K_NEWLINE;
...
function getFormUploadFile($field_name, $field_id, $name, $description = '', $onchange = '')
{
    ...
    $str = '<div class="form-row" id="div'.$field_id.'">'.K_NEWLINE;
	$str .= '<div class="input-group">'.K_NEWLINE;
    $str .= '<div class="input-group-prepend">'.K_NEWLINE;
    $str .= '<label class="input-group-text" for="'.$field_id.'" title="'.$description.'">'.$name.'</label>'.K_NEWLINE;
    $str .= '</div>'.K_NEWLINE;
    // $str .= '<span class="formw">'.K_NEWLINE;
    $str .= '<input class="form-control" type="file" name="'.$field_name.'" id="'.$field_id.'" size="20" title="'.$description.'"';
    if (!empty($onchange)) {
