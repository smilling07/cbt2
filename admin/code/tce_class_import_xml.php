    ...
    private function addAnswer()
    {
        ...
        if ($r = F_db_query($sql, $db)) {
            if ($m = F_db_fetch_array($r)) {
				...
				if($this->boolval[$this->level_data['answer']['answer_isright']]==1){
					$this->level_data['question']['answer_weight'] = 100;
				}else{
					$this->level_data['question']['answer_weight'] = 0;
				}
                $sql = 'INSERT INTO '.K_TABLE_ANSWERS.' (
					...
					'.$this->level_data['question']['answer_weight'].'
					)';
                if (!$r = F_db_query($sql, $db)) {
                    F_display_db_error(false);
